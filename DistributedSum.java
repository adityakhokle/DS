...
int[] localArray = new
int[localN]; // Local array for each processor
int localSum = 0; //Local sum for each processor
int totalSum = 0;// Scatter the input array to all processors
MPI.COMM_WORLD.Scatter(array, 0, localN, MPI.INT, localArray, 0,localN, MPI.INT, 0);// Calculate the local sum for each processor
